...
void main() {
  ...
  const servizioTest = Servizio(id: 's1', nome: 'Taglio', durataMinuti: 30, prezzo: 20);
  ...
  final nuovoAppuntamento = Appuntamento(
    ...
    orarioInizio: DateTime(2026, 3, 26, 10, 15), // Si accavalla con Mario Rossi!
    servizioScelto: servizioTest,
  );

  test('Deve lanciare SlotOccupatoException se c\'è sovrapposizione (Il Buttafuori funziona)', () async {
    // 1. ARRANGE: Prepariamo il DB finto con un appuntamento già esistente alle 10:00
    final repo = FakeAppuntamentiRepository(iniziali: [appuntamentoEsistente]);
    final validatore = ValidatoreAppuntamenti();
    final usecase = PrenotaAppuntamentoUseCase(repository: repo, validatore: validatore);
  ...
  test('Deve salvare l\'appuntamento se lo slot è completamente libero', () async {
